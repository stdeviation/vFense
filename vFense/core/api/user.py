...
class UserHandler(BaseHandler):
    ...
    def get(self, username):
        ...
        granted, status_code = (
            verify_permission_for_user(active_user, Permissions.ADMINISTRATOR)
        )
    ...
    def post(self, username):
        ...
        ###Update Views###
        view_names = self.arguments.get(ApiArguments.VIEW_NAMES, None)
        ...
        if results:
            ...
            results.generic_status_code = UserCodes.IncorrectArguments
            results.vfense_status_code = UserCodes.IncorrectArguments
    ...
    def put(self, username):
        ...
        if password and new_password:
            results = self.change_password(password, new_password)
        ###Update Personal Settings###
        fullname = self.arguments.get('fullname', None)
        if fullname:
            results = self.change_full_name(username, fullname)
        email = self.arguments.get('email', None)
        if email:
            results = self.change_email(username, email)

        current_view = self.arguments.get('current_view', None)
        if current_view:
            results = self.change_current_view(username, current_view)

        default_view = self.arguments.get('default_view', None)
        if default_view:
            results = self.change_default_view(username, default_view)
...
class UsersHandler(BaseHandler):
    ...
    def get(self):
        ...
        if granted:
            if not view_context and not all_views:
                ...
                fetch_users = (
                    RetrieveUsers(
                        count=count, offset=offset, sort=sort,
                        sort_key=sort_by, is_global=is_global
                    )
                )

            if user_name:
                results = self.get_user_by_name(fetch_users, user_name)

            elif regex:
                results = self.get_all_users_by_regex(fetch_users, regex)

            else:
                results = self.get_all_users(fetch_users)
    ...
    def post(self):
        ...
        results = (
            self.create_user(
                self, username, password,fullname, email, view_context,
